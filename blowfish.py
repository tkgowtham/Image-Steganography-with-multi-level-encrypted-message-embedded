...
Sbox1 = ["d1310ba6", "98dfb5ac", "2ffd72db", "d01adfb7", "b8e1afed",
...
Sbox2 = ["4b7a70e9", "b5b32944", "db75092e", "c4192623", "ad6ea6b0",
...
Sbox3 = ["e93d5a68", "948140f7", "f64c261c", "94692934", "411520f7",
...
Sbox4 = ["3a39ce37", "d3faf5cf", "abc27737", "5ac52d1b", "5cb0679e",
...
def genKey(key):
    ...
    for i in range(18):
        v1 = hex_to_bin(P[i])
        v2 = hex_to_bin(key[j:(j+8)])
        j = (j+8)%len(key)
        
        P[i] = bin_to_hex(xorf(v1,v2))
        #print(i," : ",P[i])
...
def addBin(a,b):
    ...
    res = res[2:]
    #print("1. ",res)
    res = ('0' * (32-(len(res)%32))) + res if len(res)%32 != 0 else res
    #print("2. ",res)
    res = bin_to_hex(res)
    return res
...
def f(l):
    a = [-1,-1,-1,-1]
    d = {0:Sbox1,1:Sbox2,2:Sbox3,3:Sbox4}
    for i in range(0,8,2):
        s = l[i:i+2]
        s = hex_to_bin(s)
        col = int(s,2)
        a[int(i/2)]= d[int(i/2)][int(col)]
    #print(a)
    ans = addBin(a[0],a[1])
    ans = bin_to_hex(xorf(hex_to_bin(ans),hex_to_bin(a[2])))
    ans = addBin(ans,a[3])
    return ans
...
def encrypt(pt,key):
    ...
    for io in pt1:
        ...
        r = io[8:]
        for i in range(16):
            x = bin_to_hex(xorf(hex_to_bin(l),hex_to_bin(P[i])))
            y = f(x)
            y = bin_to_hex(xorf(hex_to_bin(y),hex_to_bin(r)))
            l = y
            r = x
            #print("Round ",i," x: ",l," y: ",r)
        l = bin_to_hex(xorf(hex_to_bin(l),hex_to_bin(P[16])))
        #print("Round 16: ",l)
        r = bin_to_hex(xorf(hex_to_bin(r),hex_to_bin(P[17])))
        #print("Round 17: ",r)
        NE.append(r+l)
    tex=""
    for i in NE:
...
def decrypt(pt,key):
    genKey(key)
    #print("Plain text to see is ",pt)
    pt1=Segre(pt)
    NE=[]
    #print("Segregated List:",pt1)
    for io in pt1:
        l = io[:8]
        r = io[8:]
        for i in range(17,1,-1):
            ...
            r = x
            #print("Round ",i," x: ",l," y: ",r)
        l = bin_to_hex(xorf(hex_to_bin(l),hex_to_bin(P[1])))
        #print("Round 1: ",l)
        r = bin_to_hex(xorf(hex_to_bin(r),hex_to_bin(P[0])))
        #print("Round 0: ",r)
        NE.append(r+l)
    tex=""
    for i in NE:
        tex+=i
    return tex
"""genKey("aabb09182736ccdd")
print("Encryption: ")
print(encrypt("123456abcd132536"))
print("Decryption: ")
print(decrypt("d748ec383d3405f7"))"""
